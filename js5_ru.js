	...
	var apiURI2 = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&lang=ua, uk&units=metric&appid=5b58aee62c41eb64fcab16edce2e5cc1`;
		...
		$.ajax({
	      ...
	      timeout : 500,
                    ...
                    error : function(e) {
                        console.log("Error");
						$('#cityC').html('<p style="color:#ff0000";>ERROR</p><p style="color:#bef7f1";>Проверьте корректность названия</p>');
...
var m4 = znak.indexOf(data.weather[0].description);	    
    if (data.main.temp && data.sys) {
      ...
      if (data.weather) {
		...
		document.getElementById("demo27").innerHTML = "Погода: " + " " + znakRU[m4];
